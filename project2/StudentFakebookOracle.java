...
ResultSet rst = stmt.executeQuery(
    "SELECT T.FIRST, T.SECOND, U1.FIRST_NAME, U1.LAST_NAME, U1.YEAR_OF_BIRTH, U2.FIRST_NAME, U2.LAST_NAME, U2.YEAR_OF_BIRTH, P.PHOTO_ID, P.ALBUM_ID, P.PHOTO_LINK, A.ALBUM_NAME FROM ( SELECT DISTINCT LEAST(FIRST, SECOND) AS FIRST, GREATEST(FIRST, SECOND) AS SECOND"+
        " FROM (SELECT U1.USER_ID AS FIRST, U2.USER_ID AS SECOND, COUNT(T1.TAG_PHOTO_ID) FROM " + UsersTable + " U1, " + UsersTable + " U2, " + TagsTable + "  T1, " + TagsTable + "  T2 " +
            "WHERE U1.USER_ID = T1.TAG_SUBJECT_ID AND U2.USER_ID = T2.TAG_SUBJECT_ID AND T1.TAG_PHOTO_ID = T2.TAG_PHOTO_ID AND U1.GENDER = U2.GENDER AND U1.USER_ID <> U2.USER_ID AND " +
            ...
            "GROUP BY U1.USER_ID, U2.USER_ID ORDER BY COUNT(T1.TAG_PHOTO_ID) DESC, U1.USER_ID ASC, U2.USER_ID ASC) WHERE ROWNUM <= " + num +
